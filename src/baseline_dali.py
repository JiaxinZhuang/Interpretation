...
def main():
    ...
    if dataset_name == "mnist":
        ...
        trainloader = imagenet_dali.get_imagenet_iter_dali(
            mode="train", data_dir=data_dir, batch_size=batch_size,
            num_threads=num_workers, crop=re_size, device_id=0, num_gpus=1)

        valloader = imagenet_dali.get_imagenet_iter_dali(
    ...
    for epoch in range(start_epoch, n_epochs):
        if not freeze:
            net.train()
            losses = []
            for _, data in enumerate(tqdm(trainloader, ncols=70, desc=desc)):
        ...
        if freeze or epoch % eval_frequency:
            ...
            y_true = []
            y_pred = []
            for _, data in enumerate(tqdm(trainloader, ncols=70,
                                          desc="train")):
                imgs = data[0]["data"].cuda(non_blocking=True)
                target = data[0]["label"].squeeze().long().\
                    cuda(non_blocking=True)
                predict = torch.argmax(net(imgs), dim=1).cpu().data.numpy()
            ...
            y_true = []
            y_pred = []
            for _, data in enumerate(tqdm(valloader, ncols=70, desc="val")):
                imgs = data[0]["data"].cuda(non_blocking=True)
                target = data[0]["label"].squeeze().long().\
                    cuda(non_blocking=True)
                predict = torch.argmax(net(imgs), dim=1).cpu().data.numpy()
